...
function UploadFile() {
  ...
  return (
    <>
      <div className='w-3/4 mt-1 mb-1 border border-white rounded'>
        {
          files.map((file, index) => (
            <div className='bg-gray-100 rounded h-14 flex items-center mt-2 border shadow' key={index}>
              <img src="/images/file.png" width={50} height={50} alt='File' />
              <strong className="mt-1 ml-3 w-auto overflow-hidden" style={{ maxWidth: 'calc(100% - 100px)' }}>{file.name}</strong>
              <h4 className='ml-36'>to</h4>
              <div className='ml-10 w-36 '>
                <Select
                  label="Conversion Type"
                  placeholder="Select a convertion type"
                  className=""
                >
